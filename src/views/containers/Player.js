    ...
    <Fragment>
      ...
      <div className="mt-4 subheader">
		   ...
		   <div className="mx-5 mt-2">
		     <PlayerContent current={this.props.currentTrack} previous={this.props.previousTrack} next={this.props.nextTrack} />
		   </div>
...
const mapStateToProps = (state, ownProps) => {
	  ...
	  const currentTrack = state.appState.tracks.find(track => track.index == trackIndex);

	  const tracksNumber = parseInt( state.appState.tracks.length, 10 ) - 1; 
	  const previousTrackIndex = parseInt( (trackIndex - 1) < 0 ? tracksNumber : (trackIndex - 1), 10 );
	  const nextTrackIndex = parseInt( (trackIndex + 1) > tracksNumber ? 0 : (trackIndex + 1), 10 );
...
const hoc = connect(mapStateToProps, null)(Player);
